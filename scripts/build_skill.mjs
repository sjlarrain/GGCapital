#!/usr/bin/env node
/**
 * Bundle gg-crm-skill/ into public/gg-crm-skill.zip for download from the app.
 *
 * Dependency-free: writes a STORE (uncompressed) ZIP so it runs the same on
 * Windows and Linux/Vercel with no `zip` binary or npm package. Run manually
 * after editing the Skill:  `node scripts/build_skill.mjs`
 */
...
const srcDir = join(root, 'gg-crm-skill')
const outFile = join(root, 'public', 'gg-crm-skill.zip')
...
const files = walk(srcDir).map((full) => ({
  // Store with a top-level gg-crm-skill/ folder, forward-slash separators.
  name: 'gg-crm-skill/' + relative(srcDir, full).split(sep).join('/'),
