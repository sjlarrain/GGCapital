#!/usr/bin/env node
/**
 * Bundle company-enrichment-skill/ into public/company-enrichment-skill.zip
 ...
 *   node scripts/build_company_enrichment_skill.mjs
 */
...
const srcDir = join(root, 'company-enrichment-skill')
const outFile = join(root, 'public', 'company-enrichment-skill.zip')
...
const files = walk(srcDir).map((full) => ({
  // Store with a top-level company-enrichment-skill/ folder, forward-slash separators.
  name: 'company-enrichment-skill/' + relative(srcDir, full).split(sep).join('/'),
